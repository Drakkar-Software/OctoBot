...
"""Public node journal API.

Import tiers:
- Tier 1 (feature code): ``octobot.community.node_journal`` — record/read helpers and events.
- Tier 2 (infrastructure): ``octobot.community.node_journal.lifecycle``,
  ``octobot.community.node_journal.recording_context``.
- Tier 3 (internal/tests): ``octobot.community.node_journal.store``,
  ``octobot.community.node_journal.state``.

Layer rules (documentation only):
- ``journal``, ``store``, ``state`` must not import from ``recording/`` or ``lifecycle``.
- ``recording/*`` may import ``journal``, ``state``, and ``classify``; not ``lifecycle``.
- ``lifecycle`` may import ``journal``, ``recording``, and external infrastructure.
- ``journey_summary`` is read-only; no ``recording`` imports.
"""
...
on_user_data_pull_succeeded = journal_recording.on_user_data_pull_succeeded
...
reset_sync_tracker_after_node_startup = journal_recording.reset_sync_tracker_after_node_startup
