...
class ClickhouseHistoricalBackendClient(historical_backend_client.HistoricalBackendClient):
    ...
    async def fetch_candles_history(
        ...
        result = await self._client.query(
            ...
            FROM ohlcv_history
            WHERE exchange_internal_name = %s
            AND symbol = %s
            AND time_frame = %s
            ...
            """,
            [exchange, symbol, time_frame.value, first_open_time, last_open_time],
        )
        return self._format_ohlcvs(result.result_rows)
    ...
    async def fetch_candles_history_range(
        ...
        result = await self._client.query(
            ...
            AND time_frame = %s
            """,
            [exchange, symbol, time_frame.value],
        )
        return result.result_rows[0][0].timestamp(), result.result_rows[0][1].timestamp()
