...
class TrendAnalyser:
    ...
    def bollinger_trend_analysis(data_frame, delta_function):
        ...
        elif current_middle + delta >= current_value >= current_middle - delta:
        ...
        # good case: up the middle band
            ...
            return -1 * math.exp((current_value - current_middle) / delta_up) / exp_one

        # bad case: down the lower band
        elif current_middle - delta > current_value:
            return math.exp((current_middle - current_value) / delta_low) / exp_one
