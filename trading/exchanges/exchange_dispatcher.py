...
class ExchangeDispatcher(AbstractExchange):
    def __init__(self, config, exchange_manager):
        ...
        self.logger.info(f"online with REST api"
                         ...
                         f"{' and user web socket api' if self.exchange_user_web_socket else ''}")
    ...
    def _user_web_socket_available(self):
        return self.exchange_user_web_socket

    def is_user_web_socket_available(self) -> bool:
        return self._user_web_socket_available()
    ...
    async def reset_web_sockets_if_any(self):
        if self._web_socket_available() or self._user_web_socket_available():
    ...
    async def get_balance(self):
        if not self._user_web_socket_available() or not self.exchange_personal_data.get_portfolio_is_initialized():
            if not self.exchange_personal_data.get_portfolio_is_initialized():
                self.exchange_personal_data.init_portfolio()

            if self._user_web_socket_available() and self.exchange_user_web_socket.handles_balance():
    ...
    async def get_order(self, order_id, symbol=None):
        if not self._user_web_socket_available() \
    ...
    async def get_all_orders(self, symbol=None, since=None, limit=None):
        if not self._user_web_socket_available() or not self.exchange_personal_data.get_orders_are_initialized():
