...
**Backup and passphrase recovery.** After wallet setup completes, the Node UI shows a backup reminder on the OctoBots home page until you confirm you saved your seed phrase or private key offline. Settings → Export remains available anytime. If you forget your passphrase, use **Forgot your passphrase?** on the log in page: prove ownership with the same seed phrase or private key, set a new passphrase, then sign in again. Recovery requests are rate limited per client IP. Recovery is disabled when wallets are injected read-only from the environment.
