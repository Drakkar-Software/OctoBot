...
**Backup and passphrase recovery.** After wallet setup, the Node UI prompts you to export your seed phrase or private key once the wallet has been running for 48 hours, until you confirm you saved the backup offline. Settings → Export remains available anytime. If you forget your passphrase, use **Forgot your passphrase?** on the log in page: prove ownership with the same seed phrase or private key, set a new passphrase, then sign in again. Recovery is disabled when wallets are injected read-only from the environment.
