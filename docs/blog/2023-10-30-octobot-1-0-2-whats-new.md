...
## Improved TradingView connection
...
It's now possible to send a cancel order signal to cancel all current open orders for a symbol, or only to cancel an open order on a specific side using the param SIDE. More details on this can be found on [the alert format guide](/guides/octobot-interfaces/tradingview/alert-format#canceling-orders).
