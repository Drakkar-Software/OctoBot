...
/**
 * Adapt relative links to work within the docs site.
 * Handles known root-level docs and converts remaining relative .md links
 * to GitHub URLs so they don't break.
 */
function adaptLinks(content) {
  return content
    ...
    .replace(/\[([^\]]+)\]\(\.\/((?!http)[^)]+\.md)\)/g,
      '[$1](https://github.com/Drakkar-Software/OctoBot/blob/dev/$2)');
}
