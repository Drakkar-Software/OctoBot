...
4. CI: **OctoBot-CI** job **`extended_linter`** (unit tests + PR policy on `dev_requirements` only; no `cloud-install`). Package **`tests`** matrix still runs full install/tentacles as today.
...
3. Add tests under `tools/extended_linter/tests/layers/`.
