...
from core.evaluator.evaluation import EvaluationConsumer
...
class EvaluatorFactory:
    ...
    def _create_symbol_evaluators(self, exchange, crypto_currency):
        ...
        for symbol in exchange.get_exchange_manager().get_traded_pairs(cryptocurrency=crypto_currency):
            ...
            self._create_symbol_threads_managers(exchange,
                                                 symbol_evaluator,
                                                 exchange.get_exchange_manager().ohlcv_consumer)
    ...
    def _create_symbol_evaluator(self, symbol, crypto_currency_evaluator) -> SymbolEvaluator:
        ...
        symbol_evaluator.traders = self.octobot.get_exchange_traders()
        symbol_evaluator.trader_simulators = self.octobot.get_exchange_trader_simulators()
    ...
    def _create_symbol_threads_managers(self, exchange, symbol_evaluator, ohlcv_consumer):
        real_time_ta_eval_list = self._create_real_time_ta_list(exchange, symbol_evaluator)

        if self.octobot.get_time_frames():
            self._create_evaluation_consumers_with_time_frame(exchange,
                                                              symbol_evaluator,
                                                              ohlcv_consumer,
                                                              real_time_ta_eval_list)
        else:
            # without time frame
            self.evaluation_consumers[None] = \
                self._create_evaluation_consumer(time_frame=None,
                                                 ohlcv_consumer=ohlcv_consumer,
                                                 symbol_evaluator=symbol_evaluator,
                                                 exchange=exchange,
                                                 real_time_ta_eval_list=real_time_ta_eval_list)

    def _create_evaluation_consumers_with_time_frame(self,
                                                     exchange,
                                                     symbol_evaluator,
                                                     ohlcv_consumer,
                                                     real_time_ta_eval_list):
        for time_frame in self.octobot.get_time_frames():
            if self._is_time_frame_exists_in_exchange(exchange, symbol_evaluator, time_frame):
                self.evaluation_consumers[time_frame] = \
                    self._create_evaluation_consumer(time_frame=time_frame,
                                                     ohlcv_consumer=ohlcv_consumer,
                                                     symbol_evaluator=symbol_evaluator,
                                                     exchange=exchange,
                                                     real_time_ta_eval_list=real_time_ta_eval_list)
                ...
                self.logger.warning(f"{exchange.get_name()} exchange is not supporting the required time frame: "
                                    f"'{time_frame.value}' for {symbol_evaluator.symbol}.")

    @staticmethod
    def _is_time_frame_exists_in_exchange(exchange, symbol_evaluator, time_frame):
        return exchange.get_exchange_manager().time_frame_exists(time_frame.value, symbol_evaluator.symbol)
    ...
    def _create_evaluation_consumer(self,
                                    time_frame,
                                    ohlcv_consumer,
                                    symbol_evaluator,
                                    exchange,
                                    real_time_ta_eval_list) -> EvaluationConsumer:

        return EvaluationConsumer(self.octobot.get_config(),
                                  ohlcv_consumer,
                                  time_frame,
                                  symbol_evaluator,
                                  exchange,
    ...
    def _check_required_evaluators(self):
        if self.evaluation_consumers:
            etm = next(iter(self.evaluation_consumers.values()))
            if self.octobot.get_relevant_evaluators() != CONFIG_EVALUATORS_WILDCARD:
                for required_eval in self.octobot.get_relevant_evaluators():
                    ...
                    if required_class and not self._class_is_in_list(etm.evaluator.ta_eval_list, required_class):
