...
class EvaluatorFactory:
    ...
    def create_evaluation_tasks(self):
        ...
        for crypto_currency, crypto_currency_data in self.octobot.get_config()[CONFIG_CRYPTO_CURRENCIES].items():
            self._create_evaluation_task(crypto_currency, crypto_currency_data)

        self._check_required_evaluators()

    def _create_evaluation_task(self, crypto_currency, crypto_currency_data):
            ...
            if exchange.get_exchange_manager().enabled():
                ...
                if symbol in exchange.get_exchange_manager().get_traded_pairs():
                    self._create_symbol_threads_managers(exchange,
                                                         symbol_evaluator,
                                                         self._get_global_price_updater_fron_exchange_name(exchange))
    ...
    def _get_global_price_updater_fron_exchange_name(self, exchange) -> GlobalPriceUpdater:
