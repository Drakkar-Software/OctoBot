...
from .util import hollaex_grid_workflow as hollaex_grid_util
...
pytestmark = pytest.mark.xdist_group("hollaex_earncurve_functional")
...
_GRID_ACCOUNT_ID = "hollaex-earncurve-functional-grid-account"
_GRID_AUTOMATION_DISPLAY_NAME = "test_hollaex_earncurve_grid_trigger_automation"
_GRID_AUTOMATION_CONFIGURATION_ID = "d4e5f6a7-b8c9-4902-d345-67890abcdef1"


class TestTriggerTaskGridHollaexEarnCurveDbosIntegration:
    @pytest.mark.asyncio
    async def test_trigger_task_grid_hollaex_earncurve_two_iterations_then_stop(self, temp_dbos_scheduler):
        """
        End-to-end grid automation lifecycle on Hollaex (Earn Curve URL).

        Flow: create automation → wait for baseline grid → verify Earn Curve markets cached →
        forced-trigger signal (fixed price, no new orders) → stop automation → verify clean
        termination with preserved ladder → restart automation.
        """
        # Step 0 — Fixed BTC/USDT symbol and pinned close price; markets are fetched during automation
        # init (hollaex tentacle + Earn Curve URL), not in a separate preflight exchange_manager.
        grid_symbol = hollaex_grid_util.EARN_CURVE_GRID_SYMBOL
        grid_close_price = grid_sim_util.FIXED_BTC_USDC_CLOSE
        quote_symbol = hollaex_grid_util.quote_symbol_for_grid_symbol(grid_symbol)

        patched_fetch_tickers = price_mocks_module.tickers_repository_fetch_tickers_close_override(
            lambda _symbol: grid_close_price,
            traded_symbols=(grid_symbol,),
        )
        patched_fetch_ohlcv = price_mocks_module.fetch_ohlcv_side_effect_for_close_price(
            lambda: grid_close_price,
        )
        user_id = workflow_common_module.SIMULATOR_GRID_TEST_COMMUNITY_USER_ID
        protocol_account = hollaex_grid_util.protocol_account_for_hollaex_functional(
            account_id=_GRID_ACCOUNT_ID,
            quote_total=1000.0,
            quote_symbol=quote_symbol,
            account_name="Hollaex Earn Curve start/stop functional grid account",
        )
        create_user_action = hollaex_grid_util.build_create_grid_user_action(
            account_id=_GRID_ACCOUNT_ID,
        ...
        # Step 0 (continued) — Mock tickers/OHLCV and sync providers; DBOS scheduler and flow run for real.
        # ExchangeConfig carries the Earn Curve REST URL through the normal action_details → profile_data path.
        with (
            mock.patch.object(
            ...
            mock.patch(
                ...
                return_value=mock.Mock(
                    ...
                    get_exchange_config=mock.Mock(
                        return_value=hollaex_grid_util.protocol_exchange_config_for_hollaex_grid_functional(),
                    ),
                ),
            ),
            mock.patch(
                ...
                return_value=mock.Mock(
                    get_item=mock.Mock(
                        return_value=hollaex_grid_util.seeded_grid_strategy_for_hollaex_functional_wallet(
                            stored_strategy_id=hollaex_grid_util.HOLLAEX_EARNCURVE_GRID_DEFAULT_STRATEGY_ID,
                        ),
                    ),
                ),
            ),
            ...
            # Step 2 — Poll until the grid reaches baseline: 2 open buys, 2 open sells, 1 trade.
            ...
            # Step 2 (continued) — Assert baseline counts; Earn Curve markets must be in ccxt cache after normal init.
            ...
            hollaex_grid_util.assert_earncurve_markets_cached_after_automation_init()

            # Step 2 (continued) — Protocol AutomationState mirrors exchange elements while still RUNNING.
            ...
            protocol_assertions_module.assert_protocol_automation_matches_exchange_account_elements(
                ...
                expected_order_symbol=grid_symbol,
            )
            ...
            signal_user_action = workflow_common_module.build_forced_trigger_signal_user_action(
                ...
                user_action_id="ua-signal-forced-hollaex-earncurve-grid-functional",
            )
            try:
                await asyncio.wait_for(
                    workflow_common_module.enqueue_user_action_workflow_and_await_terminal_result(
            ...
            stop_user_action = workflow_common_module.build_stop_user_action(
                automation_id=parent_automation_id,
                user_action_id="ua-stop-hollaex-earncurve-grid-functional",
            )
            try:
                await asyncio.wait_for(
                    workflow_common_module.enqueue_user_action_workflow_and_await_terminal_result(
            ...
            # Step 7 — Enqueue AUTOMATION_RESTART and wait until the new workflow reaches RUNNING.
