...
class TestStartCheckAndStopDefaultConfigOctobotProcessWorkflow:
    ...
    async def test_generic_process_default_config_lifecycle(self, temp_dbos_scheduler, monkeypatch):
        ...
        with (
            ...
            stop_assert_deadline = time.monotonic() + octobot_process_workflow_module.stop_workflow_complete_seconds()
            ...
            await workflow_common_module.wait_for_stop_success_output(
                temp_dbos_scheduler,
                metadata_automation_id,
                octobot_process_workflow_module.stop_workflow_complete_seconds(),
            )
            ...
            stop_deadline = time.monotonic() + octobot_process_workflow_module.child_stop_wait_seconds()
    ...
    @pytest.mark.asyncio
    async def test_generic_process_accountless_default_config_lifecycle(
        self,
        temp_dbos_scheduler,
        monkeypatch,
        ...
        test_user_root = tmp_path / "functional_generic_process_accountless_user_root"
        ...
        try:
            with (
                ...
                try:
                    parent_automation_id = await asyncio.wait_for(
                        generic_process_octobot_module.create_generic_process_bot(
                            user_id,
                            _GENERIC_PROCESS_ACCOUNTLESS_AUTOMATION_NAME,
                        ),
                        timeout=_T_ENQUEUE_SECONDS,
                    )
                except TimeoutError as exc:
                    raise AssertionError(
                        "create_generic_process_bot timed out creating accountless automation"
                    ) from exc
                ...
                assert isinstance(
                    ...
                    octobot_protocol_models.GenericProcessConfiguration,
                )
                assert generic_process_configuration.profile_data is None

                metadata_automation_id = parent_automation_id
                ...
                assert protocol_automation.status == octobot_protocol_models.WorkflowStatus.RUNNING
                assert protocol_automation.exchange_account_ids is None
                ...
                try:
                    ...
                    raise AssertionError("execute_user_action timed out enqueueing accountless automation stop") from exc
