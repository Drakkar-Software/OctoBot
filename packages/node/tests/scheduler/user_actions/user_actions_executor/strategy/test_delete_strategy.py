...
import octobot_node.scheduler.user_actions.user_actions_executor.strategy.delete_strategy as delete_strategy_executor


class TestDeleteStrategyActionExecutorExecute:
    @pytest.mark.asyncio
    async def test_calls_provider_delete_with_wallet_and_id(self):
        ...
        with mock.patch(
            ...
            executor = delete_strategy_executor.DeleteStrategyActionExecutor(
    ...
    async def test_raises_when_payload_type_wrong(self):
        ...
        with pytest.raises(node_errors.InvalidUserActionPayloadError, match="DeleteStrategyConfiguration"):
            await executor.execute(user_action)
        provider_assertions.assert_user_action_terminal_state(
            user_action=user_action,
    ...
    async def test_strategy_not_found_error_propagates(self):
        ...
        provider_mock = mock.Mock()
        provider_mock.delete_item.side_effect = collection_errors.ItemNotFoundError("missing-strategy")
        with mock.patch(
            "octobot_sync.sync.collection_providers.StrategyProvider.instance",
            return_value=provider_mock,
        ):
