...
import octobot_node.scheduler.user_actions.user_actions_executor.automation.create_automation as create_automation_executor
...
def _stub_account_provider(account_mock, account: protocol_models.Account) -> None:
    account_executor_test_utils.stub_account_provider(account_mock, account)


def _minimal_exchange_account(*, account_id: str) -> protocol_models.Account:
    return account_executor_test_utils.minimal_exchange_account(account_id=account_id)
    ...
    def test_strategy_missing_raises_mapped_user_action_error_async(self):
        ...
        async def run_execute():
            with mock.patch(_ACCOUNT_PROVIDER_INSTANCE_PATCH) as account_mock, mock.patch(
                ...
                _stub_account_provider(
                    account_mock,
                    _minimal_exchange_account(account_id="acc-1"),
                )
