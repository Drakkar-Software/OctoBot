...
class ExchangeConfigUserActionExecutor(user_actions_executor_base.UserActionExecutor):
    """Executes user actions whose ``UserAction.result`` oneOf uses ``ExchangeConfigActionResult``."""
    ...
    def _get_error_message(
        self,
        exc: BaseException,
    ) -> protocol_models.ExchangeConfigActionResultErrorMessage:
        if isinstance(exc, collection_errors.ItemNotFoundError):
            return protocol_models.ExchangeConfigActionResultErrorMessage.EXCHANGE_CONFIG_NOT_FOUND
        if isinstance(exc, collection_errors.DuplicateItemError):
            return protocol_models.ExchangeConfigActionResultErrorMessage.DUPLICATE_ITEM
        if isinstance(
            exc,
            (
            ...
            return protocol_models.ExchangeConfigActionResultErrorMessage.INVALID_CONFIGURATION
        return protocol_models.ExchangeConfigActionResultErrorMessage(super()._get_error_message(exc))

    def _mark_user_action_completed(
        self,
        user_action: protocol_models.UserAction,
        *,
        created_exchange_config_id: typing.Optional[str] = None,
    ) -> None:
        now = timestamp_util.utc_now_datetime()
        ...
        user_action.result = protocol_models.UserActionResult(
            actual_instance=protocol_models.ExchangeConfigActionResult(
                updated_at=now,
                result_type=protocol_models.UserActionResultType.EXCHANGE_CONFIG,
                created_exchange_config_id=created_exchange_config_id,
            )
        )
