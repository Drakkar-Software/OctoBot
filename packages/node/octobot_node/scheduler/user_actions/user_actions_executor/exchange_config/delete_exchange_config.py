...
def _get_delete_exchange_config_payload(
    user_action: protocol_models.UserAction,
) -> protocol_models.DeleteExchangeConfigConfiguration:
    ...
    if wrapper is None or wrapper.actual_instance is None:
        raise node_errors.InvalidUserActionPayloadError(
            "UserAction.configuration must wrap a concrete delete-exchange-config configuration."
        )
    payload = wrapper.actual_instance
    if not isinstance(payload, protocol_models.DeleteExchangeConfigConfiguration):
        raise node_errors.InvalidUserActionPayloadError(
            "DeleteExchangeConfigActionExecutor expected DeleteExchangeConfigConfiguration, "
            f"got {type(payload).__name__}"
        )
    return payload


class DeleteExchangeConfigActionExecutor(
