...
def _get_create_exchange_config_payload(
    user_action: protocol_models.UserAction,
) -> protocol_models.CreateExchangeConfigConfiguration:
    ...
    if wrapper is None or wrapper.actual_instance is None:
        raise node_errors.InvalidUserActionPayloadError(
            "UserAction.configuration must wrap a concrete create-exchange-config configuration."
        )
    payload = wrapper.actual_instance
    if not isinstance(payload, protocol_models.CreateExchangeConfigConfiguration):
        raise node_errors.InvalidUserActionPayloadError(
            "CreateExchangeConfigActionExecutor expected CreateExchangeConfigConfiguration, "
            f"got {type(payload).__name__}"
        )
    return payload


class CreateExchangeConfigActionExecutor(
