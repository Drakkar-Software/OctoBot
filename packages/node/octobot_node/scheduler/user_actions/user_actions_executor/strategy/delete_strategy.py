...
def _get_delete_strategy_payload(
    user_action: protocol_models.UserAction,
) -> protocol_models.DeleteStrategyConfiguration:
    ...
    if wrapper is None or wrapper.actual_instance is None:
        raise node_errors.InvalidUserActionPayloadError(
            "UserAction.configuration must wrap a concrete delete-strategy configuration."
        )
    payload = wrapper.actual_instance
    if not isinstance(payload, protocol_models.DeleteStrategyConfiguration):
        raise node_errors.InvalidUserActionPayloadError(
            "DeleteStrategyActionExecutor expected DeleteStrategyConfiguration, "
            f"got {type(payload).__name__}"
        )
    return payload


class DeleteStrategyActionExecutor(
    ...
    async def _do_execute(
        ...
        delete_payload = _get_delete_strategy_payload(user_action)
        collection_providers.StrategyProvider.instance().delete_item(
            self._user_id,
            delete_payload.id,
        )
