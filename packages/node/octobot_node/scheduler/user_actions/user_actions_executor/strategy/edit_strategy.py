...
def _get_edit_strategy_payload(
...
) -> protocol_models.EditStrategyConfiguration:
    ...
    if wrapper is None or wrapper.actual_instance is None:
        raise node_errors.InvalidUserActionPayloadError(
            "UserAction.configuration must wrap a concrete edit-strategy configuration."
        )
    payload = wrapper.actual_instance
    if not isinstance(payload, protocol_models.EditStrategyConfiguration):
        raise node_errors.InvalidUserActionPayloadError(
            "EditStrategyActionExecutor expected EditStrategyConfiguration, "
            f"got {type(payload).__name__}"
        )
    return payload


class EditStrategyActionExecutor(
    ...
    async def _do_execute(
        ...
        edit_payload = _get_edit_strategy_payload(user_action)
        if edit_payload.configuration is None:
            raise node_errors.InvalidUserActionPayloadError(
                "EditStrategyConfiguration.configuration is required to update a strategy."
            )
