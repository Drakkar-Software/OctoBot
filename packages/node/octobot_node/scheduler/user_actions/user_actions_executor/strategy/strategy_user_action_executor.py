...
class StrategyUserActionExecutor(user_actions_executor_base.UserActionExecutor):
    """Executes user actions whose ``UserAction.result`` oneOf uses ``StrategyActionResult``."""
    ...
    def _get_error_message(
        self,
        exc: BaseException,
    ) -> protocol_models.StrategyActionResultErrorMessage:
        if isinstance(exc, collection_errors.ItemNotFoundError):
            return protocol_models.StrategyActionResultErrorMessage.STRATEGY_NOT_FOUND
        if isinstance(exc, collection_errors.DuplicateItemError):
            return protocol_models.StrategyActionResultErrorMessage.DUPLICATE_ITEM
        if isinstance(
            exc,
            (
            ...
            return protocol_models.StrategyActionResultErrorMessage.INVALID_CONFIGURATION
        return protocol_models.StrategyActionResultErrorMessage(super()._get_error_message(exc))

    def _mark_user_action_completed(
        self,
        user_action: protocol_models.UserAction,
        *,
        created_strategy_id: typing.Optional[str] = None,
    ) -> None:
        now = timestamp_util.utc_now_datetime()
