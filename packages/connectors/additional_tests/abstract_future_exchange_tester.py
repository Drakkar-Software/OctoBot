...
class AbstractFutureExchangeTester(AbstractExchangeTester):
    ...
    async def test_get_positions(self):
        ...
        positions = connector.get_positions([self.SYMBOL])
    ...
    async def test_get_and_set_leverage(self):
        connector = self.get_connector()
        connector.initialize()
        leverage_info = connector.get_symbol_leverage(self.SYMBOL)
        assert leverage_info is not None

        new_lev = min((leverage_info.leverage or 1) + 1, 10)
        result = connector.set_symbol_leverage(self.SYMBOL, float(new_lev), None)
    ...
    async def test_set_margin_type(self):
        connector = self.get_connector()
        connector.initialize()
        connector.set_symbol_margin_type(self.SYMBOL, False, None)
        connector.stop()

    async def test_get_funding_rate(self):
        connector = self.get_connector()
        connector.initialize()
        rate = connector.get_funding_rate(self.SYMBOL)
    ...
    async def test_get_mark_price(self):
        connector = self.get_connector()
        connector.initialize()
        price = connector.get_mark_price(self.SYMBOL)
