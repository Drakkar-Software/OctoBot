...
def create_role_resolver(
    ...
    async def role_resolver(request: Request) -> AuthResult:
        ...
        if not all([pubkey, signature, timestamp, nonce_header]):
        ...
        # Starlette keeps scope["path"] as the full concatenated path; strip root_path
        # to get the effective path that the client signed against.
