...
class AccountHistoryProvider(
    single_item_provider.SingleItemLocalCollectionProvider[protocol_models.PortfolioHistoricalValuesState],
    singleton_class.Singleton,
):
    """
    Singleton provider for per-account portfolio historical values.

    Each account is stored in its own encrypted file under
    ``<collection>/<wallet>/<account_id>.json``.
    """
    COLLECTION = sync_enums.Collections.USER_ACCOUNTS_HISTORY.value
    STATE_VERSION = sync_constants.USER_ACCOUNTS_HISTORY_STATE_VERSION
    STATE_CLASS = protocol_models.PortfolioHistoricalValuesState
