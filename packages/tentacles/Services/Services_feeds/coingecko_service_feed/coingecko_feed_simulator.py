...
import tentacles.Services.Services_feeds.coingecko_service_feed.coingecko_feed as coingecko_feed


class CoingeckoServiceFeedSimulator(coingecko_feed.CoingeckoServiceFeed):
    ...
    def _update_cache_from_events(self, topic, events):
        ...
        if topic == services_constants.COINGECKO_TOPIC_MARKETS:
            self.data_cache[topic] = [
                coingecko_feed.CoingeckoCoinMarket.from_dict(event["payload"])
                for event in events
            ]
        elif topic == services_constants.COINGECKO_TOPIC_TRENDING:
            self.data_cache[topic] = [
                coingecko_feed.CoingeckoTrendingCoin.from_dict(event["payload"])
                for event in events
            ]
        elif topic == services_constants.COINGECKO_TOPIC_GLOBAL:
            # Global data is a single object, take the latest event
            if events:
                self.data_cache[topic] = coingecko_feed.CoingeckoGlobalData.from_dict(events[-1]["payload"])
...
coingecko_feed.CoingeckoServiceFeed.SIMULATOR_CLASS = CoingeckoServiceFeedSimulator
