...
class SocialDataCollectorNamespace(abstract_websocket_namespace_notifier_module.AbstractWebSocketNamespaceNotifier):
    ...
    async def on_social_data_collector_status(
        self,
        connection: websocket_connection_module.WebSocketConnection,
    ) -> None:
        await self.registry.broadcast_async(
            "social_data_collector_status",
            self._get_social_data_collector_status(),
        )
        ...
        if self._has_clients():
            try:
                ...
                self.logger.exception(
                    error,
                    True,
                    f"Error when sending social_data_collector_status: {error}",
                )
        return False

    async def on_connect(self, connection: websocket_connection_module.WebSocketConnection) -> None:
        await self.on_social_data_collector_status(connection)
