...
export function AddWalletDialog({ onSuccess }: { onSuccess: () => void }) {
  ...
  const [createdSeed, setCreatedSeed] = useState<string | null>(null)
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (wallet) => {
      ...
      try {
        const mnemonic = await fetchSeedAfterWalletCreate(
          wallet.address,
          passphrase,
          { isOwnWallet: false },
        )
        if (mnemonic) {
          setCreatedSeed(mnemonic)
          setPhase("seed-onboarding")
          return
        }
      } catch {
        setError(
          "Wallet created but the seed phrase could not be loaded. Export it from wallet settings.",
        )
        return
      }
      setOpen(false)
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        {phase === "seed-onboarding" && createdSeed ? (
          <WalletSeedOnboardingFlow
            seed={createdSeed}
            onComplete={completeSeedOnboarding}
          />
        ) : (
          <>
            <DialogHeader>
