...
async function serverEncryptResult(
  ...
  const wrappedKey = new Uint8Array(
    await crypto.subtle.wrapKey("raw", aesKey, userRsaPublicKey, {
      name: "RSA-OAEP",
    }),
  )
...
describe("client-encryption", () => {
  beforeAll(async () => {
    ...
    ecdsaPrivateDer = await crypto.subtle.exportKey(
      "pkcs8",
      ecdsaPair.privateKey,
    )
    const rsaPrivateDer = await crypto.subtle.exportKey(
      "pkcs8",
      rsaPair.privateKey,
    )
    serverEcdsaPublicDer = await crypto.subtle.exportKey(
      "spki",
      serverEcdsaPair.publicKey,
    )
  ...
  describe("encryptAndSign", () => {
    ...
    it("content_metadata decodes to JSON with ENCRYPTED_AES_KEY_B64, IV_B64, SIGNATURE_B64", async () => {
      const { content_metadata } = await encryptAndSign(
        "hello",
        testKeys,
        serverRsaPublicPem,
      )
    ...
    it("produces different ciphertext and metadata on each call (fresh IV and AES key)", async () => {
      ...
      const { content: c1, content_metadata: m1 } = await encryptAndSign(
        msg,
        testKeys,
        serverRsaPublicPem,
      )
      const { content: c2, content_metadata: m2 } = await encryptAndSign(
        msg,
        testKeys,
        serverRsaPublicPem,
      )
    ...
    it("decrypted ciphertext matches original content (full roundtrip)", async () => {
      ...
      const { ENCRYPTED_AES_KEY_B64, IV_B64 } = JSON.parse(
        atob(content_metadata),
      )
    ...
    it("ECDSA signature over ciphertext+wrappedKey+IV verifies with ECDSA public key", async () => {
      ...
      const { ENCRYPTED_AES_KEY_B64, IV_B64, SIGNATURE_B64 } = JSON.parse(
        atob(content_metadata),
      )
      const ciphertext = b64ToBytes(content)
      const encryptedAesKey = b64ToBytes(ENCRYPTED_AES_KEY_B64)
      ...
      const p1363Sig = p256.Signature.fromBytes(derSig, "der").toBytes(
        "compact",
      ) as Uint8Array
  ...
  describe("decryptAndVerify", () => {
    it("roundtrip: server-encrypted result decrypts to original plaintext", async () => {
      ...
      const decrypted = await decryptAndVerify(
        ...
        result_metadata,
        testKeys,
        serverEcdsaPublicPem,
      )
      expect(decrypted).toBe(original)
    })

    it("decrypted result differs between calls (unique AES key and IV each time)", async () => {
      const { content: c1, result_metadata: m1 } = await serverEncryptResult(
        "msg",
        userRsaPublicKey,
        serverEcdsaPrivateKey,
      )
      const { content: c2, result_metadata: m2 } = await serverEncryptResult(
        "msg",
        userRsaPublicKey,
        serverEcdsaPrivateKey,
      )
    ...
    it("metadata is plain JSON (not base64) — passing base64-encoded metadata throws", async () => {
      ...
      await expect(
        decryptAndVerify(content, b64Metadata, testKeys, serverEcdsaPublicPem),
      ).rejects.toThrow("Invalid result metadata")
    })
    ...
    it("rejects tampered IV (signature covers IV)", async () => {
      ...
      ivBytes[0] ^= 0xff
      const tamperedMeta = JSON.stringify({
        ...meta,
        IV_B64: btoa(String.fromCharCode(...ivBytes)),
      })
      await expect(
        decryptAndVerify(content, tamperedMeta, testKeys, serverEcdsaPublicPem),
      ).rejects.toThrow("Signature verification failed")
    })
    ...
    it("rejects tampered encrypted AES key (signature covers wrapped key)", async () => {
      ...
      const tamperedMeta = JSON.stringify({
        ...meta,
        ENCRYPTED_AES_KEY_B64: btoa(String.fromCharCode(...keyBytes)),
      })
      await expect(
        decryptAndVerify(content, tamperedMeta, testKeys, serverEcdsaPublicPem),
      ).rejects.toThrow("Signature verification failed")
    ...
    it("rejects metadata missing required fields", async () => {
      await expect(
        decryptAndVerify(
          "abc",
          JSON.stringify({ ENCRYPTED_AES_KEY_B64: "x" }),
          testKeys,
          serverEcdsaPublicPem,
        ),
    ...
    it("rejects with specific error for invalid SERVER_ECDSA_PUBLIC_KEY", async () => {
      ...
      await expect(
        decryptAndVerify(content, result_metadata, testKeys, "notapem"),
      ).rejects.toThrow("Invalid SERVER_ECDSA_PUBLIC_KEY")
    })
    ...
    it("rejects with specific error for invalid USER_RSA_PRIVATE_KEY", async () => {
      ...
      const badKeys: ClientKeys = {
        ...testKeys,
        rsa_private: toPem("PRIVATE KEY", ecdsaPrivateDer),
      }
      await expect(
        decryptAndVerify(
          content,
      ...
      ).rejects.toThrow("Invalid USER_RSA_PRIVATE_KEY")
    })
    ...
    it("wrong server ECDSA key fails signature verification (not key import)", async () => {
      ...
      const wrongPublicDer = await crypto.subtle.exportKey(
        "spki",
        wrongPair.publicKey,
      )
      await expect(
        decryptAndVerify(
          ...
          result_metadata,
          testKeys,
          toPem("PUBLIC KEY", wrongPublicDer),
        ),
      ).rejects.toThrow("Signature verification failed")
    })
  ...
  describe("export-results tryDecryptAll routing", () => {
    ...
    it("entry with result_metadata but no client keys returns base row (cannot decrypt)", () => {
      ...
      const canDecrypt =
        has_result_metadata && keys !== null && serverKeys !== null
  ...
  describe("derivePublicPemsFromPrivates", () => {
    it("derived RSA public key matches the private key (encrypt + decrypt roundtrip)", async () => {
      ...
      const wrapped = await crypto.subtle.wrapKey("raw", aesKey, derivedPub, {
        name: "RSA-OAEP",
      })
    ...
    it("derived PEMs are distinct (RSA and ECDSA keys are different)", async () => {
      const { rsa_public_pem, ecdsa_public_pem } =
        await derivePublicPemsFromPrivates(testKeys)
