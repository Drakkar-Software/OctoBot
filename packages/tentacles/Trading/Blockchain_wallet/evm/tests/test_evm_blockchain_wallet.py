...
class TestCreateBlockchainDescriptor:
    def test_creates_descriptor_with_ethereum_mainnet_defaults(self):
        desc = evm_blockchain_wallet.create_blockchain_descriptor(
            evm_blockchain_wallet.ETHEREUM_MAINNET_CONFIG
        )
        assert desc.blockchain == evm_blockchain_wallet.EVMBlockchainWallet.BLOCKCHAIN
        assert desc.network == evm_blockchain_wallet.EthereumLayer1Network.ETHEREUM
        assert desc.native_coin_symbol == evm_blockchain_wallet.EthereumNativeCurrency.ETH
        assert desc.specific_config[
            evm_blockchain_wallet.EVMBlockchainSpecificConfigurationKeys.RPC_URL.value
        ] == evm_blockchain_wallet.EthereumDefaultRPCURL.ETHEREUM_MAINNET
        assert desc.tokens is evm_blockchain_wallet.ETHEREUM_MAINNET_STABLECOINS

    def test_creates_descriptor_with_polygon_mainnet_defaults(self):
        desc = evm_blockchain_wallet.create_blockchain_descriptor(
            evm_blockchain_wallet.POLYGON_MAINNET_CONFIG
        )
        assert desc.network == evm_blockchain_wallet.EthereumLayer2Network.POLYGON
        assert desc.native_coin_symbol == evm_blockchain_wallet.EthereumNativeCurrency.POL
        assert desc.specific_config[
            evm_blockchain_wallet.EVMBlockchainSpecificConfigurationKeys.RPC_URL.value
        ] == evm_blockchain_wallet.EthereumDefaultRPCURL.POLYGON_MAINNET

    def test_custom_rpc_url_overrides_default(self):
    ...
    def test_custom_tokens_override_well_known_stablecoins(self):
        custom_tokens = [
            blockchain_wallets.TokenDescriptor(
                symbol="WBTC", decimals=8, contract_address="0x" + "c" * 40
            )
        ]
        desc = evm_blockchain_wallet.create_blockchain_descriptor(
            evm_blockchain_wallet.ETHEREUM_MAINNET_CONFIG,
        ...
        assert desc.tokens == custom_tokens

    def test_empty_tokens_list_disables_token_tracking(self):
        desc = evm_blockchain_wallet.create_blockchain_descriptor(
            evm_blockchain_wallet.ETHEREUM_MAINNET_CONFIG,
            tokens=[],
        )
...
class TestCreateWalletDescriptor:
    def test_creates_descriptor_with_address(self):
        desc = evm_blockchain_wallet.create_wallet_descriptor(address=FAKE_ADDRESS)
        assert desc.address == FAKE_ADDRESS
        assert desc.private_key is None
        assert desc.mnemonic_seed is None

    def test_creates_descriptor_with_private_key(self):
        desc = evm_blockchain_wallet.create_wallet_descriptor(private_key=FAKE_PRIVATE_KEY)
        assert desc.address is None
        assert desc.private_key == FAKE_PRIVATE_KEY
        assert desc.mnemonic_seed is None

    def test_creates_descriptor_with_mnemonic(self):
        desc = evm_blockchain_wallet.create_wallet_descriptor(mnemonic_seed=FAKE_MNEMONIC)
        assert desc.address is None
        assert desc.private_key is None
        assert desc.mnemonic_seed == FAKE_MNEMONIC

    def test_creates_descriptor_with_all_fields(self):
        desc = evm_blockchain_wallet.create_wallet_descriptor(
            address=FAKE_ADDRESS,
            private_key=FAKE_PRIVATE_KEY,
            mnemonic_seed=FAKE_MNEMONIC,
        )
        assert desc.address == FAKE_ADDRESS
        assert desc.private_key == FAKE_PRIVATE_KEY
        assert desc.mnemonic_seed == FAKE_MNEMONIC
...
def _make_live_wallet(network_config, address):
    parameters = blockchain_wallets.BlockchainWalletParameters(
        blockchain_descriptor=evm_blockchain_wallet.create_blockchain_descriptor(network_config),
