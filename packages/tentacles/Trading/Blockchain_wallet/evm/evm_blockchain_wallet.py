...
def create_wallet_descriptor(
    address: typing.Optional[str] = None,
    private_key: typing.Optional[str] = None,
    mnemonic_seed: typing.Optional[str] = None,
) -> blockchain_wallets.WalletDescriptor:
    return blockchain_wallets.WalletDescriptor(
        address=address,
...
def create_blockchain_descriptor(
    network_config: EVMNetworkConfig,
    rpc_url: str = None,
    tokens: typing.Optional[list[blockchain_wallets.TokenDescriptor]] = None,
) -> blockchain_wallets.BlockchainDescriptor:
    """
    Create a BlockchainDescriptor for an EVM-compatible network.

    Uses the network_config's well-known stablecoins by default if tokens is not specified.

    Example usage:
        descriptor = create_blockchain_descriptor(POLYGON_MAINNET_CONFIG)
        descriptor = create_blockchain_descriptor(ETHEREUM_MAINNET_CONFIG, rpc_url="https://my-rpc.example.com")
        descriptor = create_blockchain_descriptor(POLYGON_MAINNET_CONFIG, tokens=[])  # no tokens tracked
    """
    return blockchain_wallets.BlockchainDescriptor(
        blockchain=EVMBlockchainWallet.BLOCKCHAIN,
        network=network_config.network,
        native_coin_symbol=network_config.native_currency,
        specific_config={
            EVMBlockchainSpecificConfigurationKeys.RPC_URL.value: rpc_url or network_config.default_rpc_url
        },
        tokens=tokens if tokens is not None else network_config.well_known_stablecoins,
    )
