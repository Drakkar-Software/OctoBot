...
async def get_amount_from_input_amount(
    ...
    if amount_type in (dsl.QuantityType.DELTA, dsl.QuantityType.DELTA_BASE):
        ...
        base_asset = parsed_symbol.base_portfolio_asset()
        quote_asset = parsed_symbol.quote_portfolio_asset()
        ...
        for symbol in context.exchange_manager.exchange_config.traded_symbols:
            assets.add(symbol.base_portfolio_asset())
            assets.add(symbol.quote_portfolio_asset())
        total_symbol_assets_holdings_value = portfolio_value_holder.get_assets_holdings_value(
            assets, parsed_symbol.base_portfolio_asset()
        )
        amount_value = total_symbol_assets_holdings_value * amount_value / trading_constants.ONE_HUNDRED
