...
from tests.test_utils.random_numbers import (
    decimal_random_quantity,
    decimal_random_price,
)
...
async def test_get_current_crypto_currencies_values(backtesting_trader):
    ...
    portfolio_value_holder.value_converter.missing_currency_data_in_exchange.remove(
        "XRP"
    )
...
async def test_get_current_holdings_values(backtesting_trader):
    ...
    exchange_manager.client_symbols.append("ETH/BTC")
    portfolio_value_holder.value_converter.missing_currency_data_in_exchange.discard(
        "ETH"
    )
    portfolio_manager.portfolio.update_portfolio_from_balance(
        {
    ...
    with (
        ...
        portfolio_value_holder.value_converter.last_prices_by_trading_pair[
            "ETH/BTC:BTC"
        ] = decimal.Decimal("50")
        portfolio_value_holder.value_converter.last_prices_by_trading_pair[
            "ETH/BTC"
        ...
        portfolio_value_holder.current_crypto_currencies_values["ETH"] = (
            decimal.Decimal("50")
        )
        ...
        portfolio_manager.handle_mark_price_update(
            "XRP/BTC", decimal.Decimal("0.00001")
        )
        assert portfolio_value_holder.get_current_holdings_values() == {
            "BTC": decimal.Decimal("10"),
            "ETH": decimal.Decimal("5000"),
...
async def test_get_origin_portfolio_current_value(backtesting_trader):
    ...
    assert (
        portfolio_value_holder.get_origin_portfolio_current_value()
        == decimal.Decimal(str(10))
    )
...
async def test_get_origin_portfolio_current_value_with_different_reference_market(
    backtesting_trader,
):
    config, exchange_manager, trader = backtesting_trader
    portfolio_manager = exchange_manager.exchange_personal_data.portfolio_manager
...
async def test_update_origin_crypto_currencies_values(backtesting_trader):
    ...
    assert (
        portfolio_value_holder.update_origin_crypto_currencies_values(
            "ETH/BTC", decimal.Decimal(str(0.1))
        )
        is True
    )
    assert portfolio_value_holder.origin_crypto_currencies_values[
        "ETH"
    ] == decimal.Decimal(str(0.1))
    assert portfolio_value_holder.value_converter.last_prices_by_trading_pair[
        "ETH/BTC"
    ] == decimal.Decimal(str(0.1))
    ...
    assert (
        portfolio_value_holder.update_origin_crypto_currencies_values(
            "BTC/USDT", decimal.Decimal(str(100))
        )
        is True
    )
    assert portfolio_value_holder.origin_crypto_currencies_values[
    ...
    assert (
        portfolio_value_holder.update_origin_crypto_currencies_values(
            "DOT/ETH", decimal.Decimal(str(0.015))
        )
        is True
    )
    assert portfolio_value_holder.origin_crypto_currencies_values[
...
async def test_sync_portfolio_current_value_using_available_currencies_values(
    backtesting_trader,
):
    config, exchange_manager, trader = backtesting_trader
    portfolio_manager = exchange_manager.exchange_personal_data.portfolio_manager
    ...
    portfolio_value_holder.sync_portfolio_current_value_using_available_currencies_values()
    assert portfolio_value_holder.portfolio_current_value == decimal.Decimal(
        str(20)
    )  # now includes USDT


@pytest.mark.parametrize(
    "backtesting_exchange_manager", ["spot", "futures"], indirect=True
)
async def test_get_holdings_ratio(backtesting_trader):
    ...
    exchange_manager.exchange_personal_data.portfolio_manager.portfolio_value_holder.portfolio_current_value = decimal.Decimal(
        "11"
    )
    exchange_manager.exchange_personal_data.portfolio_manager.portfolio.portfolio = {}
    exchange_manager.exchange_personal_data.portfolio_manager.portfolio.portfolio[
        "BTC"
    ] = personal_data.SpotAsset(
        name="BTC", available=decimal.Decimal("10"), total=decimal.Decimal("10")
    )
    exchange_manager.exchange_personal_data.portfolio_manager.portfolio.portfolio[
        "USDT"
    ] = personal_data.SpotAsset(
        name="USDT", available=decimal.Decimal("1000"), total=decimal.Decimal("1000")
    )
    usdt_removed = [False]
    ...
    with (
        ...
        assert portfolio_value_holder.get_holdings_ratio("BTC") == decimal.Decimal(
            "0.9090909090909090909090909091"
        )
        assert portfolio_value_holder.get_holdings_ratio(
            "BTC", include_assets_in_open_orders=False
        ) == decimal.Decimal("0.9090909090909090909090909091")
        assert portfolio_value_holder.get_holdings_ratio("USDT") == decimal.Decimal(
            "0.09090909090909090909090909091"
        )

        exchange_manager.exchange_personal_data.portfolio_manager.portfolio.portfolio.pop(
            "USDT"
        )
        usdt_removed[0] = True
        exchange_manager.exchange_personal_data.portfolio_manager.portfolio_value_holder.portfolio_current_value = decimal.Decimal(
        ...
        assert (
            portfolio_value_holder.get_holdings_ratio("BTC", traded_symbols_only=True)
            == constants.ZERO
        )
...
async def test_get_holdings_ratio_with_include_assets_in_open_orders(
    ...
    def mock_get_or_create_position_impl(symbol, side):
        ...
        if symbol == "BTC/USDT:BTC":
            mock_pos.margin = decimal.Decimal(
                "10"
            )  # equivalent to the 10 BTC in the spot portfolio
            mock_pos.initial_margin = decimal.Decimal("10")
            mock_pos.is_idle.return_value = False
        if symbol == "BTC/USDT:USDT":
            mock_pos.margin = decimal.Decimal(
                "1000"
            )  # 1000 USDT margin, converts to 1 BTC at 1000 USDT/BTC
            mock_pos.initial_margin = decimal.Decimal("1000")
            mock_pos.is_idle.return_value = False
        elif symbol == "ETH/USDT:BTC":
            mock_pos.margin = (
                decimal.Decimal("1") if has_eth_position else constants.ZERO
            )  # 1 BTC margin (settlement is BTC)
            mock_pos.initial_margin = (
                decimal.Decimal("1") if has_eth_position else constants.ZERO
            )
            mock_pos.is_idle.return_value = False
    ...
    with (
        ...
        assert portfolio_value_holder.get_holdings_ratio(
            "USDT", include_assets_in_open_orders=False
        ) == decimal.Decimal("0.09090909090909090909090909091")

        # no open order
        assert portfolio_value_holder.get_holdings_ratio(
            "BTC", include_assets_in_open_orders=True
        ) == decimal.Decimal("0.9090909090909090909090909091")
        assert portfolio_value_holder.get_holdings_ratio(
            "USDT", include_assets_in_open_orders=True
        ) == decimal.Decimal("0.09090909090909090909090909091")
        ...
        order_1 = mock.Mock(
            ...
            total_cost=decimal.Decimal("1000"),
        ...
        order_2 = mock.Mock(
            ...
            total_cost=decimal.Decimal("100"),
            status=enums.OrderStatus.OPEN,
            side=enums.TradeOrderSide.BUY,
        )
        order_3 = mock.Mock(
            order_id="3",
            symbol="XRP/ETH",
            origin_quantity=decimal.Decimal("100"),
            total_cost=decimal.Decimal("0.001"),
            status=enums.OrderStatus.OPEN,
            side=enums.TradeOrderSide.SELL,
        )
        # open orders
        await exchange_manager.exchange_personal_data.orders_manager.upsert_order_instance(
            order_1
        )
        await exchange_manager.exchange_personal_data.orders_manager.upsert_order_instance(
            order_2
        ...
        assert portfolio_value_holder.get_holdings_ratio(
            "ETH", include_assets_in_open_orders=False
        ) == decimal.Decimal("0")
        assert portfolio_value_holder.get_holdings_ratio(
            "USDT", include_assets_in_open_orders=False
        ) == decimal.Decimal("0.09090909090909090909090909091")
        ...
        assert portfolio_value_holder.get_holdings_ratio(
            "ETH", include_assets_in_open_orders=True
        ...
        exchange_manager.exchange_personal_data.portfolio_manager.portfolio.portfolio[
            ...
            name="ETH", available=decimal.Decimal("10"), total=decimal.Decimal("10")
        )
        ...
        assert portfolio_value_holder.get_holdings_ratio(
            ...
            "0.1000083333333333333333333333"
        )  # ETH now taken into account in total value
...
async def test_get_traded_assets_holdings_value(
    backtesting_trader, unit, coins_whitelist
):
    config, exchange_manager, trader = backtesting_trader
    portfolio_manager = exchange_manager.exchange_personal_data.portfolio_manager
    portfolio_value_holder = portfolio_manager.portfolio_value_holder
    ...
    exchange_manager.client_symbols.append("BTC/USDT")
    portfolio_value_holder.value_converter.last_prices_by_trading_pair["BTC/USDT"] = (
        decimal.Decimal("1000")
    )
    portfolio_value_holder.value_converter.missing_currency_data_in_exchange.discard(
        "USDT"
    ...
    result = portfolio_value_holder.get_traded_assets_holdings_value(
        unit, coins_whitelist
    )
...
@pytest.mark.parametrize(
    "currency,traded_symbols_only,include_assets_in_open_orders,coins_whitelist,expected_ratio_zero",
    [
...
async def test_get_holdings_ratio_from_portfolio(
    backtesting_trader,
    currency,
    traded_symbols_only,
...
@pytest.mark.parametrize(
    "coins_whitelist,traded_symbols_only,use_portfolio_current_value",
    [
        (None, False, True),
        (None, True, False),
...
async def test_get_total_holdings_value(
    backtesting_trader,
    coins_whitelist,
    traded_symbols_only,
    use_portfolio_current_value,
):
    config, exchange_manager, trader = backtesting_trader
    portfolio_manager = exchange_manager.exchange_personal_data.portfolio_manager
    portfolio_value_holder = portfolio_manager.portfolio_value_holder
    ...
    result = portfolio_value_holder._get_total_holdings_value(
        coins_whitelist, traded_symbols_only
    )
...
@pytest.mark.parametrize(
    "currency,order_symbol,order_side,order_quantity,order_cost,expected_delta",
    [
        (
            "BTC",
            "BTC/USDT",
...
async def test_get_orders_delta(
    backtesting_trader,
    currency,
    order_symbol,
    order_side,
...
@pytest.mark.parametrize(
    "order_side,order_quantity,filled_quantity,order_price,expected_value",
    [
...
async def test_get_open_orders_value_for_symbol(
    backtesting_trader,
    order_side,
    order_quantity,
    filled_quantity,
    ...
    order.update(
        ...
        if order_side == enums.TradeOrderSide.BUY
        else enums.TraderOrderType.SELL_LIMIT,
        symbol=test_symbol,
