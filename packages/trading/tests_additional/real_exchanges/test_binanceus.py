...
class TestBinanceUSRealExchangeTester(real_exchange_tester.RealExchangeTester):
    ...
    async def test_get_order_book(self):
        # Exact small book, then probe that a large custom limit expands depth beyond default.
        await self.assert_get_order_book(limit=5)
        await self.assert_order_book_custom_limit(
            450,
            supports_custom_limit=True,
