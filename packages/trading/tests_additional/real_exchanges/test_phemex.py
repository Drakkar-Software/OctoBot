...
class TestPhemexRealExchangeTester(real_exchange_tester.RealExchangeTester):
    EXCHANGE_NAME = "phemex"
    SYMBOL = "BTC/USDT"
    SYMBOL_2 = "ETH/USDT"
    SYMBOL_3 = "XRP/USDT"
    ALLOWED_TIMEFRAMES_WITHOUT_CANDLE = 1
    ...
    async def test_time_frames(self):
        await self.assert_time_frames([
            ...
            commons_enums.TimeFrames.THREE_HOURS,
    ...
    async def test_get_kline_price(self):
        ...
        kline_start_time = kline_price[0][commons_enums.PriceIndexes.IND_PRICE_TIME.value]
        assert kline_start_time >= self.get_time() - self.get_allowed_time_delta()

    async def test_get_order_book(self):
        await self.assert_get_order_book(limit=30)
