...
class TestWeexRealExchangeTester(real_exchange_tester.RealExchangeTester):
    EXCHANGE_NAME = "weex"
    SYMBOL = "BTC/USDT"
    SYMBOL_2 = "ETH/USDT"
    SYMBOL_3 = "DOGE/USDT"
    ...
    async def test_active_symbols(self):
        await self.inner_test_active_symbols(2303, 3131)
    ...
    async def test_get_symbol_prices(self):
        await self.assert_get_symbol_prices(
            default_allowed_lengths=(301,),
            tested_limit=200,
        )
        # CCXT spot fetchOHLCV hard cap is 300; Weex may include one extra in-progress candle (301).
        symbol_prices = await self.get_symbol_prices(limit=1000)
        assert symbol_prices is not None
        assert len(symbol_prices) in (300, 301)
    ...
    async def test_get_price_ticker(self):
        def extra_checks(ticker):
            # Validate OctoBot ticker column types (prices as Decimal, timestamp as float, etc.).
            # => more checks than required
        ...
        def _price_ticker_expectations() -> real_exchange_tester.TickerRequiredExpectations:
            # Default ticker expectations treat bid/ask volumes as absent (NONE).
            # Weex spot tickers include both; override only those fields (Bingx pattern).
            ticker_expect = real_exchange_tester.TickerExpect
            return real_exchange_tester.TickerRequiredExpectations(
                bid_volume=ticker_expect.TRUTHY,
                ask_volume=ticker_expect.TRUTHY,
            )
