...
const OCTOBOT_DEFAULT_PORT: u16 = 5001;
...
impl Backend for BinaryBackend {
    ...
    async fn start(&self, spec: &InstanceSpec, progress: Option<&ProgressSender>) -> Result<()> {
        ...
        // Always log OctoBot stdio so startup errors are visible.
        ...
        // Wait for OctoBot's HTTP server to respond, or detect a startup crash.
        // PyInstaller binaries take many seconds to extract + initialize before either
        // serving HTTP or exiting; a fixed short delay is never sufficient.
        let probe_url = spec
            .runtime_options
            .get("http_probe")
            ...
            .and_then(|v| v.as_str())
            .map(str::to_string)
            .unwrap_or_else(|| format!("http://127.0.0.1:{}/", OCTOBOT_DEFAULT_PORT));

        const POLL_INTERVAL: Duration = Duration::from_millis(500);
        let startup_timeout_secs = spec
            .runtime_options
            .get("startup_timeout_secs")
            .and_then(|v| v.as_u64())
            .unwrap_or(60);
        let deadline = tokio::time::Instant::now() + Duration::from_secs(startup_timeout_secs);
        send_progress(progress, "Waiting for OctoBot to start");
        loop {
            tokio::time::sleep(POLL_INTERVAL).await;
            if let Ok(Some(status)) = child.try_wait() {
                remove_pid_file(&pid_path).ok();
                return Err(LauncherError::Backend(format!(
                    "OctoBot exited during startup with {}",
                    status
                )));
            }
            let probe = run_http_probe(&probe_url).await;
            if probe.status_code.is_some() {
                send_progress(progress, "OctoBot is ready");
                break;
            }
    ...
    async fn stop(&self, spec: &InstanceSpec, timeout: Duration) -> Result<()> {
        let pid = {
            let inner = self.inner.lock().await;
            if inner.child.is_none() {
                let pid_path = pid_file_path(&spec.data_dir);
                return if let Ok(Some(pid)) = read_pid_file(&pid_path) {
    ...
    async fn restart(
        ...
        match self.stop(spec, timeout).await {
    ...
    async fn update(
        ...
        match self.stop(spec, Duration::from_secs(10)).await {
            Ok(()) | Err(LauncherError::NotRunning) => {}
            Err(e) => return Err(e),
        }
    ...
    async fn remove(&self, spec: &InstanceSpec, purge: bool) -> Result<()> {
        let data_dir = {
            let inner = self.inner.lock().await;
            inner.data_dir.clone().or_else(|| Some(spec.data_dir.clone()))
        };

        match self.stop(spec, Duration::from_secs(30)).await {
            Ok(()) | Err(LauncherError::NotRunning) => {}
            Err(e) => return Err(e),
        }
...
mod tests {
    ...
    fn make_spec(data_dir: &Path) -> InstanceSpec {
        InstanceSpec {
            ...
            auto_restart: false,
            auto_update: false,
            runtime_options: serde_json::json!({"startup_timeout_secs": 2}),
        }
    }
    ...
    mod lifecycle {
        ...
        async fn start_when_already_running_returns_error() {
            ...
            backend.stop(&spec, Duration::from_secs(5)).await.unwrap();
        }
        ...
        async fn stop_when_not_running_returns_error() {
            ...
            let result = backend.stop(&spec, Duration::from_secs(5)).await;
    ...
    mod signal {
        ...
        async fn sigterm_then_sigkill() {
            ...
            backend
                .stop(&spec, Duration::from_millis(100))
                .await
                .unwrap();
    ...
    mod stdio {
        ...
        async fn capture_stdio_appends_to_log() {
            ...
            spec.runtime_options = serde_json::json!({"capture_stdio": true, "startup_timeout_secs": 2});

            backend.prepare(&spec, None).await.unwrap();
            backend.start(&spec, None).await.unwrap();
            tokio::time::sleep(Duration::from_millis(200)).await;
            let _ = backend.stop(&spec, Duration::from_secs(5)).await;
