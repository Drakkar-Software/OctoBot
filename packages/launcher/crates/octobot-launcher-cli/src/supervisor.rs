...
use octobot_launcher_config::{
    DesiredState, LauncherConfig, BOOTSTRAP_TOKEN_FILENAME, LAUNCHER_PID_FILENAME, LOCK_FILENAME,
    SOCKET_FILENAME, Store,
};
...
pub async fn run_supervisor(config: LauncherConfig, store: Arc<Store>) -> Result<()> {
    ...
    let pid_path = config.launcher.data_root.join(LAUNCHER_PID_FILENAME);
