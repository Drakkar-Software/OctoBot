...
pub trait Backend: Send + Sync + 'static {
    ...
    async fn stop(&self, id: InstanceId, timeout: std::time::Duration) -> Result<()>;
    async fn restart(
        &self,
        spec: &InstanceSpec,
...
pub mod mock {
    ...
    impl Backend for MockBackend {
        ...
        async fn stop(&self, id: InstanceId, _timeout: std::time::Duration) -> Result<()> {
            let mut lock = self.inner.lock().await;
        ...
        async fn remove(&self, id: InstanceId, _purge: bool) -> Result<()> {
...
mod tests {
    ...
    async fn mock_backend_lifecycle() {
        ...
        backend
            .stop(spec.id, std::time::Duration::from_secs(5))
            .await
            .unwrap();
        assert!(matches!(
            backend.get_state(spec.id).await.unwrap(),
        ...
        backend.remove(spec.id, false).await.unwrap();
